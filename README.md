...
- **It refuses, before sending anything, commits it cannot represent.** `createCommitOnBranch` carries a path and base64 content but no file mode, so a mode change, a new file that is executable locally, a symlink or a submodule in any position, a merge commit, an empty commit, or a commit over the payload ceiling is refused with the commit and path named. Editing an existing executable is fine: the mutation keeps the mode the entry already has. Nothing is sent for the whole range. Use a plain local `git push` for those. See ADR 0002.
